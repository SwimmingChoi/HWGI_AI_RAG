...
class FAQ:
    ...
    # [2팀] FAQ 처리 로직 (FAQ 데이터셋 질문들 중 사용자 입력 질문과 의미적으로 동일한 질문이 있는지 확인하고, 적절한 값 반환)
    """
    FAQ를 처리하기 위한 클래스입니다.
    ...
    def find_similar_sentence(self, input_sentence, question_dictionary):
        """
        [2팀] 사용자 질문(input sentence)과 FAQ질문사전(question_dictionary)에 있는 질문들 간의 의미적 동일성을 판단합니다.
...
동일한 문장이 여러개 있다면 가장 유사한 문장의 인덱스를 반환해주세요. """
    ...
    def run_faq(self, question, question_dictionary, faq_data):
        """
        [2팀] 사용자 질문이 FAQ데이터베이스 내의 질문들과 의미적으로 동일한지 판단하고, 의미적으로 동일한 질문이 존재한다면 FAQ 데이터베이스 내 해당당 답변을 찾아 반환합니다.
        ...
        try:
            # [2팀] STEP 1: FAQ 데이터셋 질문들 중 사용자 입력 질문과 의미적으로 동일한 질문이 있는지 확인
            # → OPEN AI LLM 사용
            # → 존재한다면 질문 인덱스, 없다면 -1 반환
            similar_index = int(checker.find_similar_sentence(question, qd))
            if similar_index != -1:
                # [2팀] STEP 2: FAQ 데이터셋 내 해당 인덱스의 답변 반환 
                # → FAQ 데이터셋 내 페이지, 조항 정보가 있다면 함께 반환
